...
EOF
            umask "${prev_umask}"
            echo -e "${red}写入失败 PostgreSQL credentials to ${PG_CRED_FILE}${plain}" >&2
            return 1
        fi
        umask "${prev_umask}"
    fi
...
ensure_pg_client() {
    ...
    echo -e "${yellow}正在安装 PostgreSQL 客户端工具 (pg_dump/pg_restore) for in-panel backup...${plain}" >&2
    case "${release}" in
        ubuntu | debian | armbian)
...
setup_ssl_certificate() {
    ...
    if ! command -v ~/.acme.sh/acme.sh &> /dev/null; then
        install_acme
        if [ $? -ne 0 ]; then
            echo -e "${yellow}安装 acme.sh 失败, skipping SSL setup${plain}"
            return 1
        fi
    fi
    ...
    if [ $? -ne 0 ]; then
        echo -e "${yellow}证书申请失败 for ${domain}${plain}"
...
setup_ip_certificate() {
    ...
    while true; do
        if is_port_in_use "${WebPort}"; then
            ...
            read -rp "Enter another port for acme.sh standalone listener (留空取消): " alt_port
            alt_port="${alt_port// /}"
            if [[ -z "${alt_port}" ]]; then
    ...
    if [[ "$NONINTERACTIVE" == "1" ]]; then
        ...
        while true; do
            ...
            if ! is_domain "$domain"; then
                echo -e "${red}域名格式无效: ${domain}. Please enter a valid domain name.${plain}"
                continue
            fi
...
prompt_and_setup_ssl() {
    ...
    echo -e "${green}1.${plain} Let's Encrypt 域名证书 (90-day validity, auto-renews)"
    ...
    if [[ "$NONINTERACTIVE" == "1" ]]; then
        ...
        read -rp "请选择一个选项 (default 2 for IP): " ssl_choice
    ...
    case "$ssl_choice" in
        ...
        2)
            ...
            local ipv6_addr=""
            prompt_or_default ipv6_addr "Do you have an IPv6 address to include? (留空跳过): " "" XUI_SSL_IPV6
        ...
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            if [[ "$db_choice" == "2" ]]; then
                ...
                while [[ -z "$xui_dsn" ]]; do
                    ...
                    echo ""
                    echo -e "  1) 本机安装 PostgreSQL and create a dedicated user/db (recommended)"
                    echo -e "  2) 使用已有 PostgreSQL 服务器 (enter DSN)"
...
EOF
            ...
            if [[ "$db_choice" == "2" ]]; then
                ...
                echo -e "  ${blue}${SSL_SCHEME}://${SSL_HOST}:${config_port}/${config_webBasePath}${plain} → Backup & 恢复"
                echo -e "${yellow}  备份 downloads a pg_dump .dump file; 恢复 reloads it via pg_restore.${plain}"
            fi
...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "https://api.github.com/repos/Fourgetu/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "https://api.github.com/repos/Fourgetu/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
        ...
        curl -4fLR --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 300 -o ${xui_folder}-linux-$(arch).tar.gz https://github.com/Fourgetu/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
    ...
    curl -4fLRo /usr/bin/x-ui-temp https://raw.githubusercontent.com/Fourgetu/3x-ui-cn-installer/main/x-ui-cn.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/Fourgetu/3x-ui/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/Fourgetu/3x-ui/main/x-ui.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/Fourgetu/3x-ui/main/x-ui.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/Fourgetu/3x-ui/main/x-ui.service.rhel > /dev/null 2>&1
                    ;;
            esac
...
│  ${blue}x-ui enable${plain}       - 启用开机自启 on OS Startup   │
│  ${blue}x-ui disable${plain}      - 禁用开机自启 on OS Startup  │
