...
confirm_restart() {
    confirm "重启 the panel, Attention: Restarting the panel will also restart xray" "y"
    if [[ $? == 0 ]]; then
        restart
    else
        show_menu
...
update_menu() {
    ...
    curl -fLRo /usr/bin/x-ui https://raw.githubusercontent.com/Fourgetu/3x-ui-cn-installer/main/x-ui-cn.sh
...
reset_user() {
    ...
    read -rp "是否禁用当前配置的两步验证？ (y/n): " twoFactorConfirm
...
check_config() {
    ...
    if [[ -n "$existing_cert" ]]; then
        ...
        if [[ -n "$cert_sans" && $(echo "$cert_sans" | wc -l) -gt 1 ]]; then
            echo -e "${yellow}该证书还覆盖：${plain} $(echo "$cert_sans" | grep -vx "$domain" | tr '\n' ' ')"
        fi
    else
        echo -e "${red}⚠ WARNING: 未配置 SSL 证书！${plain}"
        ...
        if [[ "$gen_ssl" == "y" || "$gen_ssl" == "Y" ]]; then
            ...
            if [[ $? -eq 0 ]]; then
                ...
                echo -e "${yellow}You can try again via option 19 (SSL 证书管理).${plain}"
                start 0 > /dev/null 2>&1
            fi
        else
            echo -e "${yellow}访问地址： http://${server_ip}:${existing_port}${existing_webBasePath}${plain}"
            echo -e "${yellow}For security, please configure SSL certificate using option 19 (SSL 证书管理)${plain}"
        fi
    fi
}
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        echo -e "端口已设置, Please restart the panel now, and use the new port ${green}${port}${plain} to access web panel"
        confirm_restart
    fi
}
...
update_shell() {
    curl -fLRo /usr/bin/x-ui -z /usr/bin/x-ui https://raw.githubusercontent.com/Fourgetu/3x-ui-cn-installer/main/x-ui-cn.sh
    if [[ $? != 0 ]]; then
        echo ""
...
run_geo_update() {
    ...
    if [[ $? -ne 0 ]]; then
        ...
        echo -e "${green}${name} 已经是最新，无需重启。, restart is not needed.${plain}"
    fi
}
...
ssl_cert_issue_main() {
    echo -e "${green}\t1.${plain} 申请 SSL (Domain)"
    ...
    echo -e "${green}\t6.${plain} 申请 SSL for IP Address (6-day cert, auto-renews)"
    echo -e "${green}\t0.${plain} 返回主菜单"
    ...
    case "$choice" in
        ...
        6)
            ...
            echo -e "${yellow}80 端口必须开放并可从公网访问.${plain}"
...
ssl_cert_issue_for_ip() {
    ...
    local ipv6_addr=""
    read -rp "Do you have an IPv6 address to include? (留空跳过): " ipv6_addr
...
ssl_cert_issue() {
    ...
    while true; do
        ...
        if ! is_domain "$domain"; then
            LOGE "域名格式无效: ${domain}. Please enter a valid domain name."
            continue
        fi
    ...
    if ! command -v fail2ban-client &> /dev/null; then
        ...
        case "${release}" in
            ...
            alpine)
                ...
                ;;
            *)
                echo -e "${red}不支持的操作系统. Please check the script and install the necessary packages manually.${plain}\n"
                return 1
                ;;
        esac
        ...
        2)
            ...
            case "${release}" in
                ...
                alpine)
                    ...
                    ;;
                *)
                    echo -e "${red}不支持的操作系统. Please uninstall Fail2ban manually.${plain}\n"
                    exit 1
                    ;;
            esac
...
show_banlog() {
    ...
    if [[ -f "$system_log" ]]; then
        ...
        grep "3x-ipl" "$system_log" | grep -E "Ban|Unban" | tail -n 10 || echo -e "${yellow}否 recent system ban activities found${plain}"
        echo ""
    fi
    ...
    if [[ -f "${iplimit_banned_log_path}" ]]; then
        ...
        if [[ -s "${iplimit_banned_log_path}" ]]; then
            grep -v "INIT" "${iplimit_banned_log_path}" | tail -n 10 || echo -e "${yellow}否 ban entries found${plain}"
        else
...
pg_install_server_action() {
    ...
    echo -e "${yellow}使用选项 2 可迁移 SQLite 数据并切换面板到 PostgreSQL。${plain}"
}
...
migrate_to_postgres() {
    ...
    if [[ -z "$dsn" ]]; then
        echo ""
        echo -e "${green}\t1.${plain} 本机安装 PostgreSQL and create a dedicated user/db (recommended)"
        echo -e "${green}\t2.${plain} 使用已有 PostgreSQL 服务器 (enter DSN)"
...
migrate_db_prompt() {
    ...
    read -rp "输出文件 (leave empty to auto-name next to input): " output
